...
@section('content')
    ...
    <div id="kt_app_content_container" class="t_container">
        ...
        <div class="card shadow-sm" style="border-top: 3px solid #007bff;">
            @php
                generateCardHeader('users.add_permissions', 'admin.users.index', ' ');
            @endphp

            <form action="{{ route('admin.users.update_permissions', $admin->id) }}" method="POST">
                @csrf

                <div class="m-3 d-flex align-items-center">
                    <label class="form-check-label fs-5 fw-semibold" for="checkAllPermissions">
                        {{ trans('users.check_all_permissions') }}
                    </label>
                        ...
                        <div class="col-md-4 mb-3">
                            <div class="card">
                                <div class="card-header d-flex justify-content-between align-items-center">
                                    <span class="section-title fw-bold">{{ trans('users.'.$sectionName) }}</span>
                                    ...
                                    @foreach ($permissions as $permission)
                                        <div class="form-check">
                                            <input class="form-check-input larger-checkbox permission-checkbox"
                                                ...
                                                id="{{ $permission->name }}"
                                                @if ($admin->hasPermissionTo($permission->name)) checked @endif>
