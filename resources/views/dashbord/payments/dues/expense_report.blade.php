    ...
    <div class="card shadow-sm" style="border-top: 3px solid #007bff;">
        ...
        <div class="card-body pt-0">
            <div class="row" style="margin-top:20px">
                <div class="col-md-3">
                    <label for="first_name" class="form-label">{{ trans('payment.band') }}</label>
                    <div class="input-group flex-nowrap">
                        ...
                        <select class="form-select rounded-start-0" onchange="get_company(this.value)"
                            ...
                            @foreach($bnod as $item)
                            <option
                                value="{{$item->id}}" {{ old('band_id') == $item->id ? 'selected' : '' }}>{{$item->title}}</option>
                            @endforeach
                        </select>
                    </div>
                    @error('band_id')
...
<script>
    function get_revenue_report() {
        ...
        $.ajax({
            url: '{{route('admin.payment.get_expense_report')}}',
            type: 'get',
            data: {
                from_date: fromDate,
                band_id: band_id,
